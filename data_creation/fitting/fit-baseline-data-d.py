...
# One stored-baseline fit per data-d file (baselines are identical within a file).
PATH = "data-d"
OUT_YAML = "fitting/baseline_fits_data_d.yaml"
OUT_STATS_YAML = "fitting/baseline_fit_stats_data_d.yaml"
OUT_STATS_DIR = "fitting/baseline_fit_stats_data_d"
...
def _fit_seven(
    ...
    def model_free(
        ...
        return baseline_fit(f, *_assemble_params(fit_params, circuit_params))

    free_p0 = tuple(fit_p0[name] for name in FIT_PARAM_NAMES)
    ...
    return _assemble_params(fit_params, circuit_params)
...
def fit_baseline(
    ...
    if fixed_circuit_params is not None:
        ...
        params = _fit_seven(freq_mhz, baseline, fixed_circuit_params, start)
    else:
        params, _ = curve_fit(
            baseline_fit,
            freq_mhz,
...
def fit_baseline_with_retries(
    freq_mhz: np.ndarray,
    baseline: np.ndarray,
) -> tuple[np.ndarray, float, float] | None:
    """Try default 21-param fit, then alternate 7-param seeds if NRMSE is too high."""
    candidates: list[tuple[np.ndarray, float, float]] = []

    try:
        candidates.append(fit_baseline(freq_mhz, baseline))
    except Exception:
        pass
    ...
    default_circuit = {
        name: float(value)
        for name, value in zip(CIRCUIT_PARAM_NAMES, DEFAULT_CIRC_CONSTS, strict=True)
    }
    alt_fit_p0s = [
        default_fit_p0,
        {**default_fit_p0, "U": 2.0, "phi_const": 0.5},
        {**default_fit_p0, "U": 8.0, "phi_const": 1.0},
        {**default_fit_p0, "U": 1.0, "trim": 0.2},
    ]
    for start in alt_fit_p0s:
        try:
            params = _fit_seven(freq_mhz, baseline, default_circuit, start)
            nrmse, rmse = _fit_errors(freq_mhz, baseline, params)
            candidates.append((params, nrmse, rmse))
            if nrmse < MAX_NRMSE:
                # Polish with a full 21-param fit seeded from this result.
                try:
                    polished, _ = curve_fit(
                        baseline_fit,
                        freq_mhz,
                        baseline,
                        p0=tuple(float(v) for v in params),
                        maxfev=MAX_FEV,
                    )
                    candidates.append((polished, *_fit_errors(freq_mhz, baseline, polished)))
                except Exception:
                    pass
                break
        except Exception:
    ...
    params, nrmse, rmse = min(candidates, key=lambda item: item[1])
    if nrmse >= MAX_NRMSE:
        return None
...
def print_summary(summary: dict) -> None:
    ...
    print("mode:    one fit per file using the first stored baseline event")
    print(f"gate:    MAX_NRMSE={MAX_NRMSE}")
...
def first_baseline_index(records: list[dict], freq_mhz: np.ndarray) -> int | None:
    """Return the first event whose stored baseline is a usable nonzero curve."""
    for index, record in enumerate(records):
        if VOLTAGE_KEY not in record:
            continue
        signal = np.asarray(record[VOLTAGE_KEY], dtype=np.float64)
        if signal.ndim == 1 and len(signal) == len(freq_mhz) and bool(np.any(signal)):
            return index
    return None
...
def main() -> None:
    ...
    for filename in tqdm(txt_files, desc="Files"):
        ...
        if not records or "freq_list" not in records[0]:
            continue
        ...
        index = first_baseline_index(records, freq_mhz)
        n_skipped += len(records) if index is None else max(0, len(records) - 1)
        if index is None:
            continue

        signal = np.asarray(records[index][VOLTAGE_KEY], dtype=np.float64)
        try:
            fitted = fit_baseline_with_retries(freq_mhz, signal)
        except Exception:
            n_skipped += 1
            continue
        if fitted is None:
