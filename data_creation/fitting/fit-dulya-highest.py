...
# Peak Polarize event of each sign in every data-d file -> one YAML.
PATH = "data-d"
OUT_YAML = "fitting/dulya_fits_highest.yaml"
OUT_STATS_YAML = "fitting/dulya_fit_stats_highest.yaml"
...
def _record_pol(record: dict) -> float | None:
    try:
        pol = float(record["pol"])
...
def is_valid_polarize_event(record: dict, freq_mhz: np.ndarray) -> bool:
    ...
    if VOLTAGE_KEY not in record or "pol" not in record or "cc" not in record:
        return False
    if _record_pol(record) is None:
        return False
    signal = np.asarray(record[VOLTAGE_KEY], dtype=np.float64)
    return signal.ndim == 1 and len(signal) == len(freq_mhz) and bool(np.any(signal))
...
def peak_pol_candidates(records: list[dict], freq_mhz: np.ndarray) -> list[int]:
    ...
    for index, record in enumerate(records):
        if not is_valid_polarize_event(record, freq_mhz):
            continue
        pol = _record_pol(record)
        if pol is None:
            continue
        valid.append((index, pol))

    if not valid:
        return []

    physical = [item for item in valid if abs(item[1]) <= 1.0]
    pool = physical if physical else valid

    positive = sorted((item for item in pool if item[1] > 0.0), key=lambda item: item[1], reverse=True)
    negative = sorted((item for item in pool if item[1] < 0.0), key=lambda item: item[1])
    if not positive and not negative:
        extreme = max(pool, key=lambda item: abs(item[1]))
        return [extreme[0]]
...
def main() -> None:
    ...
    for filename in tqdm(txt_files, desc="Fitting Dulya peak pol"):
        ...
        mask = wing_mask(len(freq_mhz))
        candidates = peak_pol_candidates(records, freq_mhz)
        n_skipped += max(0, len(records) - len(candidates))
        file_events: dict[int, dict] = {}
        fitted_positive = False
        fitted_negative = False
        ...
        for index in candidates:
            ...
            if pol is None:
                n_skipped += 1
                continue
            if pol > 0.0 and fitted_positive:
                continue
            if pol < 0.0 and fitted_negative:
                continue

            event = try_fit_event(record, freq_mhz, mask)
            if event is None:
                n_skipped += 1
                continue
            ...
            n_fitted += 1
            if pol > 0.0:
                fitted_positive = True
            elif pol < 0.0:
                fitted_negative = True
            if fitted_positive and fitted_negative:
                break
